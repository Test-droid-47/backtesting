    ...
    def _discover_and_load_data(self, data_path: str = None) -> bool:
        ...
        if 'fear_greed' in discovered:
            try:
                ...
                ts_col = next((c for c in fg_df.columns if 'time' in c or 'date' in c), None)
                val_col = next((c for c in fg_df.columns if 'fear' in c or 'greed' in c or 'value' in c), None)
