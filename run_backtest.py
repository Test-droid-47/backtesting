...
# ============================================================================
# FILE DISCOVERY - CORRECTED FOR EXACT FILE NAMES
# ============================================================================
class FileDiscovery:
    @staticmethod
    def find_models(base_path: str = '.') -> Dict[str, str]:
        ...
        for search_path in search_paths:
            ...
            for file in os.listdir(search_path):
                ...
                # Features JSON
                elif file == 'selected_features.json':
...
class MultiModelConsensus:
    ...
    Ensemble, PPO Actor, PPO Critic = Helpers (confidence boost)
    """
    
    def __init__(self):
        # Model weights - LSTM is BOSS
        self.lstm_weight = 0.50      # BOSS - 50% weight
        self.ensemble_weight = 0.25   # Helper 1 - 25%
        self.ppo_actor_weight = 0.15  # Helper 2 - 15%
        self.ppo_critic_weight = 0.10 # Helper 3 - 10%
    ...
    def get_lstm_signal(self, lstm_output) -> Tuple[int, float, float]:
        """Extract signal, confidence, buy_prob from LSTM BOSS output"""
        buy_prob = 0.5
        ...
        try:
            if isinstance(lstm_output, (list, tuple)):
                if len(lstm_output) >= 3:
                    ...
                    if len(probs) >= 3:
                        buy_prob = probs[2]  # Assuming [sell, hold, buy]
                        sell_prob = probs[0]
                    else:
                ...
                if lstm_output.ndim == 1:
                    ...
                    elif len(lstm_output) >= 2:
                        buy_prob = lstm_output[1]
                    else:
                        buy_prob = lstm_output[0]
                else:
                    # 2D array
                    if lstm_output.shape[1] >= 3:
                        buy_prob = float(np.mean(lstm_output[:, 2]))
                    elif lstm_output.shape[1] >= 2:
                        buy_prob = float(np.mean(lstm_output[:, 1]))
                    else:
                        buy_prob = float(np.mean(lstm_output[:, 0]))
                sell_prob = 1 - buy_prob
                
            elif isinstance(lstm_output, (int, float)):
                buy_prob = float(lstm_output)
                sell_prob = 1 - buy_prob
            else:
                buy_prob = 0.5
                sell_prob = 0.5
            ...
            # Determine signal (2=Buy, 1=Hold, 0=Sell)
    ...
    def get_ensemble_signal(self, ensemble_pred) -> Tuple[int, float]:
        """Extract signal from ensemble helper"""
        try:
    ...
    def get_ppo_signal(self, ppo_model, default_confidence: float = 0.5) -> Tuple[int, float]:
        """
        Extract confidence from PPO models (Actor or Critic)
        PPO models are helpers - they just provide confidence boost
        """
        try:
            if ppo_model is None:
                return 1, default_confidence
                
            # Try to get output from PPO model
            # Since we don't know exact output format, we try multiple approaches
            output = None
            
            # Approach 1: Simple prediction with dummy input
            try:
                if hasattr(ppo_model, 'predict'):
                    # Create dummy input (shape depends on model)
                    dummy_input = np.zeros((1, 10))  # 10 features dummy
                    output = ppo_model.predict(dummy_input, verbose=0)
            except:
                pass
                
            # Approach 2: If it has call method
            if output is None and hasattr(ppo_model, '__call__'):
                try:
                    dummy_input = np.zeros((1, 10))
                    output = ppo_model(dummy_input)
                except:
                    pass
            ...
            if output is not None:
                if isinstance(output, (list, tuple)):
                    # Take first output element
                    first_out = output[0]
                    if isinstance(first_out, np.ndarray):
                        val = float(np.mean(first_out))
                    else:
                        val = float(first_out)
                elif isinstance(output, np.ndarray):
                    val = float(np.mean(output))
                else:
                    val = float(output)
                    
                # Convert to confidence between 0.3 and 0.9
                confidence = 0.5 + (val * 0.4) if abs(val) <= 1 else 0.5
                confidence = min(max(confidence, 0.3), 0.9)
                
                # PPO models are helpers - they don't override, just provide confidence
                # So signal is always NEUTRAL/HOLD
                return 1, confidence
                
        except Exception as e:
            logger.debug(f"PPO model extraction error: {e}")
            
        return 1, default_confidence
    
    def calculate_consensus(self, lstm_signal, lstm_confidence, lstm_buy_prob,
                           ensemble_signal, ensemble_confidence,
                           ppo_actor_confidence, ppo_critic_confidence) -> Dict:
        """
        Calculate weighted consensus with LSTM override capability
        PPO models only contribute confidence, not direction
        """
        
        # Weighted voting - only LSTM and Ensemble decide direction
        buy_score = 0.0
        sell_score = 0.0
        total_weight = 0.0
        
        # LSTM BOSS vote (has direction)
        if lstm_signal == 2:
        ...
        # PPO models - confidence boost only (neutral direction)
        ppo_confidence_boost = (ppo_actor_confidence + ppo_critic_confidence) / 2
        ppo_boost_weight = self.ppo_actor_weight + self.ppo_critic_weight
        
        # Apply confidence boost to both buy and sell (amplifies existing signal)
        buy_score *= (1 + ppo_confidence_boost * 0.3)
        sell_score *= (1 + ppo_confidence_boost * 0.3)
        total_weight += ppo_boost_weight * 0.5
        ...
        # Check agreement between LSTM and Ensemble only
        if lstm_signal == ensemble_signal:
            agreement = 0.9
        elif abs(lstm_signal - ensemble_signal) == 2:
            agreement = 0.2  # Opposite signals
        else:
            agreement = 0.5  # One hold, other directional
            
        # Apply PPO confidence boost to agreement
        agreement = min(agreement + (ppo_confidence_boost * 0.1), 1.0)
        
        # LSTM override logic (when LSTM very confident and models disagree)
        lstm_override = False
        final_signal = 1
        final_confidence = consensus_score
        
        if lstm_confidence > self.lstm_override_threshold and agreement < 0.5:
            # LSTM BOSS overrides disagreement
        ...
        # Apply PPO confidence boost to final confidence
        final_confidence = min(final_confidence * (1 + ppo_confidence_boost * 0.2), 0.95)
...
class BacktestRunner:
    ...
    def _discover_and_load_models(self, models_dir: str = None) -> bool:
        ...
        # Load PPO Actor Helper (optional)
        if 'ppo_actor' in discovered:
            print(f"\n  Loading PPO Actor Helper...")
            try:
                from tensorflow.keras.models import load_model
        ...
        # Load PPO Critic Helper (optional)
        if 'ppo_critic' in discovered:
            print(f"\n  Loading PPO Critic Helper...")
            try:
                from tensorflow.keras.models import load_model
        ...
        if 'features' in discovered:
            ...
            logger.info(f"  ✅ Features file found")

        # LSTM BOSS is mandatory
    ...
    def _generate_predictions(self) -> bool:
        ...
        # LSTM BOSS predictions
        if 'lstm' in self.models:
            print(f"\n  🦁 LSTM BOSS: Running inference on {len(X)} samples...")
            try:
                ...
                logger.info(f"  ✅ LSTM BOSS predictions generated")
            except Exception as e:
                logger.error(f"  ❌ LSTM BOSS prediction failed: {e}")
                return False

        # Ensemble Helper predictions
        if 'ensemble' in self.models:
            print(f"\n  🤝 Ensemble Helper: Running inference...")
            try:
                for i in tqdm(range(len(df)), desc="  Ensemble", leave=False):
                    try:
                        ...
                        elif hasattr(self.models['ensemble'], 'predict_proba'):
                            prob = self.models['ensemble'].predict_proba(row.reshape(1, -1))[0][1] if hasattr(row, 'reshape') else 0.5
                        else:
                            prob = 0.5
                ...
                logger.info(f"  ✅ Ensemble Helper predictions generated")
            except Exception as e:
        ...
        # PPO Helper (Actor & Critic) - optional, for confidence only
        # Pre-compute PPO confidences once for all rows (to avoid per-row overhead)
        ppo_actor_confidence = 0.5
        ppo_critic_confidence = 0.5
        
        if 'ppo_actor' in self.models:
            try:
                _, confidence = self.consensus.get_ppo_signal(self.models['ppo_actor'], 0.5)
                ppo_actor_confidence = confidence
                logger.info(f"  ✅ PPO Actor confidence: {ppo_actor_confidence:.3f}")
            except Exception as e:
                logger.warning(f"  ⚠️ PPO Actor confidence extraction failed: {e}")
                
        if 'ppo_critic' in self.models:
            try:
                _, confidence = self.consensus.get_ppo_signal(self.models['ppo_critic'], 0.5)
                ppo_critic_confidence = confidence
                logger.info(f"  ✅ PPO Critic confidence: {ppo_critic_confidence:.3f}")
            except Exception as e:
                logger.warning(f"  ⚠️ PPO Critic confidence extraction failed: {e}")
                
        # Store PPO confidences in df (same for all rows - global confidence boost)
        df['ppo_actor_confidence'] = ppo_actor_confidence
        df['ppo_critic_confidence'] = ppo_critic_confidence
    ...
    def _run_backtest(self) -> Dict:
        ...
        for i in tqdm(range(len(df)), desc="  Backtest progress", leave=False):
            ...
            # PPO confidence values (global boost)
            ppo_actor_confidence = float(df['ppo_actor_confidence'].iloc[0]) if 'ppo_actor_confidence' in df.columns else 0.5
            ppo_critic_confidence = float(df['ppo_critic_confidence'].iloc[0]) if 'ppo_critic_confidence' in df.columns else 0.5
            ...
            consensus = self.consensus.calculate_consensus(
                ...
                ensemble_signal, ensemble_confidence,
                ppo_actor_confidence, ppo_critic_confidence
            )
            
            df.at[i, 'consensus_signal'] = consensus['signal']
            df.at[i, 'consensus_confidence'] = consensus['confidence']
            
            # Get signal for trading
            if consensus['signal'] == 2:  # BUY
                final_signal = 1
                final_quality = consensus['confidence']
            elif consensus['signal'] == 0:  # SELL
                final_signal = 0
                final_quality = consensus['confidence']
            else:  # HOLD
                # If in position, hold; if not, stay out
            ...
            if final_signal == 1 and position == 0 and final_quality >= 0.5:
                ...
                trades.append({
                    ...
                    'lstm_override': consensus['lstm_override']
                })
        ...
        return {
            ...
            'lstm_overrides': self.consensus.get_stats().get('lstm_overrides', 0),
            'portfolio': portfolio,
            'trades': trades
        }
    ...
    def run(self, models_dir: str = None, data_path: str = None) -> Dict[str, Any]:
        ...
        # Print consensus-specific info
        if results.get('lstm_overrides', 0) > 0:
            print(f"\n  🔵 LSTM BOSS overrode helpers {results['lstm_overrides']} times")
